...
- CLI args: `init.minimal.args.iterate()` is POSIX-only (`@compileError` on Windows — no flat argv). For cross-platform use `init.minimal.args.iterateAllocator(gpa)` + `defer it.deinit()` (deinit is a no-op on POSIX, frees the parsed cmdline on Windows). Returned arg slices point into the iterator's buffer, so it must outlive their use.
- translate-c against the TARGET ABI in build.zig: `const tc = b.addTranslateC(.{ .root_source_file, .target, .optimize, .link_libc = true });` then `tc.addIncludePath(b.path(dir))`, `tc.defineCMacro("NAME", "1")`, and `const m = tc.addModule("name")` (public) / `tc.createModule()` (private). Pass `.target = target` (NOT host) so struct layout/enum width match the shipped binary. The step is cache-keyed + reused across targets (see the Vulkan bindings section in build.zig). For Vulkan specifically, define `VK_NO_PROTOTYPES` so only `PFN_*` typedefs are emitted (no extern fn decls to link) — load entry points via `std.DynLib` + `vkGetInstanceProcAddr`.
- C enums from translate-c are plain int aliases, not Zig enums: `pub const enum_VkResult = c_int;` with enumerators as `pub const VK_SUCCESS: c_int = 0;`. So `result == c.VK_SUCCESS` just works (both `c_int`); flag-bit consts are `c_int` even when the struct field is a `u32` flag word, so OR them and `@intCast` to assign. Opaque handles are `?*struct_X_T` (compare to `null`, not a literal). String-macro consts (`VK_*_EXTENSION_NAME`) are 0-term Zig string literals → coerce to `[*c]const u8`.
- `@min(runtime_u32, comptime_array_len)` narrows the RESULT type to the smallest int that fits the comptime bound (e.g. `@min(n, 64)` → `u7`), so `var got = @min(...); &got` yields `*u7` and fails to pass as `*u32`. Annotate the binding: `var got: u32 = @min(n, buf.len);`.
- File mmap is POSIX-only (`std.posix.mmap`). For Windows, `std.os.windows.kernel32` does NOT wrap the mapping calls — declare `extern "kernel32" fn CreateFileMappingW/MapViewOfFile/UnmapViewOfFile(...) callconv(.winapi)` yourself (only `CloseHandle` is wrapped). `std.Io.File.handle` is `std.posix.fd_t` = `*anyopaque` = `HANDLE` on Windows, so it feeds `CreateFileMappingW` directly. The view survives closing both the file handle and mapping handle; only `UnmapViewOfFile` releases it. Both paths live behind a comptime-known `is_windows` const so the untaken branch never analyzes (see `src/core/platform/platform.zig`).
- `std.DynLib` has NO Windows support in 0.16: its `InnerType` switch has no `.windows` arm, so Windows falls into `else => @compileError("unsupported platform")` (only ElfDynLib/DlDynLib remain). Wrap the OS primitive yourself behind `is_windows`: `extern "kernel32" fn LoadLibraryA([*:0]const u8) ?windows.HMODULE` / `GetProcAddress(HMODULE, [*:0]const u8) ?*anyopaque` / `FreeLibrary(HMODULE) BOOL` (none are wrapped), delegating to `std.DynLib.openZ`/`.lookup`/`.close` elsewhere. In the lookup wrapper, `@ptrCast` the proc address to `T` via an explicit `const f: T = @ptrCast(p)` BEFORE returning `?T` — casting straight into the `?T` slot makes `@ptrCast` see a double-optional and reject it as a non-pointer target (see `src/vk/vtbl.zig`).
- slangc emits Make/GCC-style depfiles with Windows paths escaped as `C\:\\Users\\...` (escaped colon + doubled backslashes). Zig's `Build/Cache/DepTokenizer` only unescapes spaces — it passes `\:`/`\\` through literally, so the `Step.Run` `stat` of that path fails with `error.BadPathName` (NOT `InvalidDepFile`). clang's depfiles use single backslashes + unescaped colons, which Zig handles; slangc's don't. Don't use `addDepFileOutputArg` with slangc on Windows — instead register all shader sources as `addFileInput`s so any change rebuilds every kernel (see `addSlangKernel` in build.zig).
- Android cross-compile (`-Dtarget=aarch64-linux-android.<api>`): Zig has NO bundled bionic — the exe link fails with "unable to provide libc" unless you supply the NDK. Don't use a global `--sysroot` (it redirects the native host codegen tools too and breaks them). Instead, scoped to the exe only (see the android branch in build.zig): (1) `exe.setLibCFile()` with a written libc.conf whose `crt_dir`/`include_dir`/`sys_include_dir` point into `<NDK>/toolchains/llvm/prebuilt/<host>/sysroot/usr/{lib/<triple>/<api>,include[,/<triple>]}`; (2) `exe.root_module.addLibraryPath(<that lib dir>)` — crt_dir alone is NOT added to the `-l` search path; (3) `exe.linkage = .dynamic` — the NDK ships only `.so` (no `.a`), so the default static link can't resolve `-lc/-lm/-ldl`. Result is a PIE with interpreter `/system/bin/linker64`. Runs from `adb shell` in `/data/local/tmp` and `dlopen`s the device's `/system/lib64/libvulkan.so` (validated: OnePlus Adreno 750 ran full GPT-2 inference). Install the NDK with the SDK `sdkmanager "ndk;<ver>"`.